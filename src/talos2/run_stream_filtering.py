"""
A streaming filtering and labelling process for small variants
...
@dataclass
class DeNovoConfig:
    """Thresholds for the naive de novo configuration test, from RunSmallFiltering.de_novo."""
    ...
    def from_config(cls) -> 'DeNovoConfig':
        conf = config_retrieve(['RunSmallFiltering', 'de_novo'], {})
        return cls(
...
def passes_ac_filter(
    ...
    """
    Callset-frequency filter, never removes variants with few instances, overridden by a ClinVar Pathogenic annotation.
    """
    if clinvar_talos:
        return True
...
def prepare_output_header(reader: VCF):
    ...
    csq_contents = '|'.join(config_retrieve(['RunSmallFiltering', 'csq_string']))
