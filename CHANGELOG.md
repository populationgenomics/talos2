...
* `MakeScatterRegions` now stages only the `.tbi` - `bcftools index -s` reads contig, length and record count from the index alone, so the whole-callset VCF is no longer localised into that task. This task only scatters across chr1-22, X, Y. Mito requires a separate annotation flow.
